...
### file_stat
检查工作区文件/路径是否存在与类型（只读）。引用文件前必须先用它确认存在。
参数：
  - path
...
5. **文件引用纪律（防幻觉）**：引用工作区文件前，必须先调用 file_stat 确认它存在；
   file_stat 返回 not found 的文件就是不存在，不得在规划、总结或代码中把它当作事实引用；
   确认不存在的文件，要么重新生成它，要么在 expected_outcomes 中如实说明。
...
## 文件引用纪律（防幻觉）
引用工作区文件前，必须先调用 file_stat 确认它存在；
file_stat 返回 not found 的文件就是不存在，不得在规划、总结或代码中把它当作事实引用；
确认不存在的文件，要么重新生成它，要么如实说明。
